...
int intSize() {
  ...
  long int firstAddy;
  long int secAddy;
  firstAddy = (long int)intPtr1;
  secAddy = (long int)intPtr2;
  return secAddy - firstAddy;
}
...
int doubleSize() {
  ...
  long int firstAddy;
  long int secAddy;
  firstAddy = (long int) doubPtr1; 
  secAddy = (long int) doubPtr2;
  return secAddy - firstAddy;
}
...
int pointerSize() {
 ...
 // int  ptrPtr2 = ptrArray + 1;
  long int first;
  long int sec;
    first = (long int) ptrPtr1;
    sec = (long int) ptrPtr2;
   return sec - first;
}
...
int changeValue() {
  ...
  int * intPtr2 = intArray + 5;
  * intPtr2  = 255 + 194;
  return (long int)(* intPtr2);
}
...
int withinSameBlock(int * ptr1, int * ptr2) {
  // Your code here
 int one = ((long int)ptr1 - 1) >> 6;
 int two = ((long int)ptr2 - 1) >> 6;
 return !(one ^ two);
}
...
int withinArray(int * intArray, int size, int * ptr) {
  // Your code here
  int result = size * 4 - 1;
  int difference = (long int)ptr - (long int)intArray;
  return !(difference>>31) * !((result - difference)>>31); 
}
...
int endianExperiment(int * ptr) {
  ...
  // Your code here
  // 449449 = 1101101101110101001 in binary and 0x0006DBA9 in hex
  *bytePtr = 0xA9;
   bytePtr++;
...
void selectionSort(int arr[], int arrLength) {
  ...
  for(i = 0;i < arrLength - 1;i++){ // to n - 1
  	min_index = i;
  for(j = i + 1; j < arrLength; j++){ // to n - 1
